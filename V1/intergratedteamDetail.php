...
$data = [
    ...
    "keywordMapList"=>["fields"=>"content_id","source_type"=>"team","source_id"=>$tid,"page_size"=>100,"content_type"=>"information","list"=>["page_size"=>6,"fields"=>"id,title,create_time"]],
	"intergratedTeamList"=>["page"=>1,"page_size"=>7,"game"=>$config['game'],"fields"=>'tid,team_name,logo',"except_team"=>$tid,"rand"=>1,"cacheWith"=>"currentPage","cache_time"=>86400*7],
    "video_list"=>["dataType"=>"informationList","game"=>$config['game'],"page"=>1,"page_size"=>6,"type"=>"7","fields"=>"id,title,logo,site_time,create_time","cache_time"=>3600,"cacheWith"=>"currentPage"],
    ...
    <div class="row">
      <div class="col-lg-3 col-4">
        <div class="t_p">
            <?php if(isset($return['defaultConfig']['data']['default_team_img'])){?>
                <img lazyload="true" data-original="<?php echo $return['defaultConfig']['data']['default_team_img']['value'];?>" src="<?php echo $return['intergratedTeam']['data']['logo'];?>" title="<?php echo $return['intergratedTeam']['data']['team_name'];?>" />
            <?php }else{?>
                <img src="<?php echo $return['intergratedTeam']['data']['logo'];?>" title="<?php echo $return['intergratedTeam']['data']['team_name'];?>" />
            <?php }?>
        </div>
      </div>
      <div class="col-lg-9 col-8">
        <div class="w_z">
          ...
          <div class="j_s">
            <ul>
              <li><span>国家</span><?php echo $return['intergratedTeam']['data']['location'];?></li>
              <li><span>英文名</span><?php echo $return['intergratedTeam']['data']['en_name'];?></li>
              <li><span>游戏战绩</span><?php echo implode("/",json_decode($return['intergratedTeam']['data']['race_stat'],true));?></li>
            </ul>
          </div>
          <div class="j_j"><?php echo $return['intergratedTeam']['data']['description'];?></div>
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="xg_zd">
        <ul class="row">
            <?php $i = 1;foreach($return['intergratedTeamList']['data'] as $team){?>
                <li><a href="<?php echo $config['site_url']."/team/".$team['tid'];?>">
                        <div class="t_b">
                            <?php if(isset($return['defaultConfig']['data']['default_team_img'])){?>
